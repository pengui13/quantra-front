      ...
      <div className="max-w-[1280px] mx-auto flex items-center justify-between px-4 sm:px-6 py-4">
        ...
        {isAuthed && (
          <nav className="hidden lg:flex items-center gap-6">
            <NavItem href={`/${currentLocale}/dashboard`} icon={<LayoutDashboard size={18} />} label="Dashboard" />
            <NavItem href={`/${currentLocale}/about-us`} icon={<CircleHelp size={18} />} label="About Us" />
            <NavItem href={`/${currentLocale}/deposit`} icon={<LogIn size={18} />} label="Deposit" />
            <NavItem href={`/${currentLocale}/withdraw`} icon={<LogOut size={18} />} label="Withdraw" />
            <NavItem href={`/${currentLocale}/staking`} icon={<TrendingUp size={18} />} label="Staking" />
            <NavItem href={`/${currentLocale}/otc`} icon={<Shuffle size={18} />} label="OTC" />
          </nav>
        )}
          ...
          {!isAuthed ? (
            ...
            <>
              ...
              <div className="hidden lg:flex items-center gap-2">
                <BalanceDropdown currentLocale={currentLocale} />
                <CoinModal />
                <button
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          ...
          >
            ...
            >
              {/* Nav Links */}
              <MobileNavItem href={`/${currentLocale}/dashboard`} icon={<LayoutDashboard size={18} />} label="Dashboard" />
              <MobileNavItem href={`/${currentLocale}/about-us`} icon={<CircleHelp size={18} />} label="About Us" />
              <MobileNavItem href={`/${currentLocale}/deposit`} icon={<LogIn size={18} />} label="Deposit" />
              <MobileNavItem href={`/${currentLocale}/withdraw`} icon={<LogOut size={18} />} label="Withdraw" />
              <MobileNavItem href={`/${currentLocale}/staking`} icon={<TrendingUp size={18} />} label="Staking" />
              <MobileNavItem href={`/${currentLocale}/otc`} icon={<Shuffle size={18} />} label="OTC" />

              {/* Balance & CoinModal in dropdown */}
              <div className="flex flex-col gap-2 mt-4">
                <BalanceDropdown isMobile currentLocale={currentLocale} />
                <CoinModal />
                ...
                >
                  <Globe className="inline w-4 h-4 mr-2 text-[#36C6E0]" /> Switch Language
                </button>
                <button
                  onClick={handleLogout}
                  className="px-4 py-2 border border-[#36C6E0]/30 rounded-lg text-white hover:border-[#36C6E0] hover:text-[#36C6E0] transition flex items-center"
                >
                  <LogOut className="inline w-4 h-4 mr-2 text-white" /> Logout
                </button>
              </div>
            </motion.div>
          </motion.nav>
...
function BalanceDropdown({ isMobile, currentLocale }) {
          ...
          <AnimatePresence>
            {open && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -10 }}
                className="mt-2 w-full bg-[#0d0d0d] border border-gray-700 rounded-lg shadow-lg p-3 flex flex-col gap-2"
              >
                <a
                  href={`/${currentLocale}/transactions`}
                  className="flex items-center gap-2 px-3 py-2 hover:bg-[#36C6E0]/10 rounded text-white text-sm"
